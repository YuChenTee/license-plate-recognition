...
output_csv = (
    r"D:\Lecture notes and exercises\Computer Vision\license-plate-recognition\yolov5\labels_easyocr_improved.csv"
)
        ...
        for result in results:
            ...
            for i in range(len(result["rec_texts"])):
                try:
                    ...
                    print(
                        f"    PaddleOCR detected: '{text}' (score: {score:.2f}) at ({x_center:.1f},{y_center:.1f}) size: {width}x{height}"
                    )
        ...
        for config in configs:
            try:
                ...
                if texts:
                    ...
                    if len(cleaned_text) > len(best_text) or (
                        len(cleaned_text) == len(best_text) and avg_confidence > best_confidence
                    ):
                        best_text = cleaned_text
                        best_confidence = avg_confidence / 100.0  # Convert to 0-1 range
                        print(
                            f"    Tesseract detected: '{cleaned_text}' (confidence: {avg_confidence:.1f}%) with config: {config}"
                        )
...
def ensemble_ocr(img):
    ...
    if easy_text:
        results.append({"engine": "easyocr", "text": easy_text, "length": len(easy_text), "confidence": easy_conf})

    # Run PaddleOCR
    paddle_text, paddle_conf = run_paddle_ocr(processed_img)
    if paddle_text:
        results.append({"engine": "paddle", "text": paddle_text, "length": len(paddle_text), "confidence": paddle_conf})
    ...
    if tesseract_text:
        results.append(
            {"engine": "tesseract", "text": tesseract_text, "length": len(tesseract_text), "confidence": tesseract_conf}
        )
